...
void DrawBow(CSprite@ this, CBlob@ blob, ArcherInfo@ archer, f32 armangle, const u8 arrowType, Vec2f armOffset)
{
	...
	if (!archer.has_arrow || archer.charge_state == ArcherParams::no_arrows || archer.charge_state == ArcherParams::legolas_charging)
	{
		...
		if (blob.get_s16("archer_charge_time") == ArcherParams::ready_time)
		{
			animname = "fired";
		}

		u16 frontframe = 0;
		f32 temp = Maths::Min(blob.get_s16("archer_charge_time"), ArcherParams::ready_time);
	...
	{
		u16 frontframe = 0;
		f32 temp = blob.get_s16("archer_charge_time");
		f32 ready_tween = temp / ArcherParams::ready_time;
		armangle = armangle * ready_tween;
		armOffset = Vec2f(-1.0f, 4.0f + config_offset + 2.0f * (1.0f - ready_tween));
	...
	{
		u16 frontframe = Maths::Min((blob.get_s16("archer_charge_time") / (ArcherParams::shoot_period_1 + 1)), 2);
		setArmValues(frontarm, true, armangle, 0.1f, "charge", Vec2f(-4.0f * sign, 0.0f), armOffset);
		frontarm.animation.frame = frontframe;

		const f32 arrowangle = (blob.get_s16("archer_charge_time") > ArcherParams::shoot_period_2) ? armangle + -2.5f + float(XORRandom(500)) / 100.0f : armangle; // shiver arrow when fully charged
