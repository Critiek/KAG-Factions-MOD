...
void AttackBlob(CBlob@ blob, CBlob @target)
{
	...
	bool shieldTime = gametime - blob.get_u32("shield time") < 10;
	...
	{
		int r = getGameTime() % 32;
		if ( r < 2 && (!backOffTime || difficulty > 4))
		{
	...
	{
		// start attack
		if (XORRandom((target.isKeyPressed(key_action1) ? 3: 10)) == 0 && (getGameTime() - blob.get_u32("attack time")) > 10) //End the slash when they stop shielding, start slashing when they've not.
		{
	...
	else if (targetDistance < 50.0f && getGameTime() - blob.get_u32("attack time") < 15) // release and attack when appropriate
	{
...
////////////////!(target.isKeyPressed(key_action2) && (blob.get_u32("attack time") - getGameTime()) < 15)