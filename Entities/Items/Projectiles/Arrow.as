...
void onTick(CBlob@ this)
{
	...
	if(this.hasTag("Narayanastra") && shape.isStatic() && getGameTime() % 4 == 0) //Less Cheeky breeky
	{
		if(getNet().isServer())
		{
			...
			if(arrow !is null)
			{
				arrow.setVelocity(Vec2f(XORRandom(14) - 7, XORRandom(8) - 3));
				arrow.SetDamageOwnerPlayer(this.getDamageOwnerPlayer());
				arrow.server_SetTimeToDie(4);
			}
		}
	}
	else if(this.hasTag("Narayanastra") && this.getVelocity().y > 1)
	{
		shape.SetStatic(true);
		this.set_Vec2f("lock", this.getPosition());
	}
	const u8 arrowType = this.get_u8("arrow type");
	...
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		...
		else
		{
			...
			if (dmg > 0.0f)
			{
				...
				if(this.hasTag("Oliknockback"))
				{
					blob.setVelocity((initVelocity*17 / blob.getMass()) + blob.getVelocity());
				}
			}
		}
...
f32 getArrowDamage(CBlob@ this, f32 vellen = -1.0f)
{
	...
	{
		return 1.0f * this.get_f32("dmgmult");
	}
	...
	return 0.5f * this.get_f32("dmgmult");
}
