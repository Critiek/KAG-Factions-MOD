	...
	void handlePixel(SColor pixel, int offset)
	{
		...
		if(alpha < 255)
		{
			...
			else if(rgb == lever)
			{
				...
				offsets[autotile_offset].push_back(offset);
				// ToDo: start on or off based on alpha
			}
			...
			else if(rgb == magazine)
			{
				...
				CBlob@ item = server_CreateBlob(items[alpha], 255, position);
		...
		else if (pixel == color_fish_school)
		{
			spawnBlob( map, "fishyfac", offset, 255);
			offsets[autotile_offset].push_back( offset );
		}
		...
		else if(pixel == color_rpg_spawn)
		{
			AddMarker( map, offset, "rpg_spawn");
		}
		else if(pixel == color_rpg_stop)
		{
			AddMarker( map, offset, "rpg_stop");
		}
...
void getInfoFromBlob(CBlob@ this, SColor &out color, Vec2f &out offset)
{
	...
	else if(name == "magazine")
	{
		...
		if(inventory.isFull())
		{
			...
			s8 element = items.find(blob.getName());
			if(element != -1)
			{
				alpha = element;
			}
